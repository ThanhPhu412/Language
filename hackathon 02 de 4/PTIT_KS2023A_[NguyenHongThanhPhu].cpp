...
int cdCount = 2; // Số lượng CD mặc định đã có trong danh sách
...
// Hàm thêm mới CD vào cuối danh sách
void addNewCD() {
    if (cdCount < MAX_CD) {
        struct CD newCD;
        ...
        printf("Noi dung: ");
        scanf(" %99[^\n]", newCD.content); // Them dau cach truoc % để loại bỏ ký tự '\n' còn lại trong buffer
        newCD.status = true; // Mặc định CD mới được thêm vào là đang chờ thuê
        newCD.id = cdCount + 1; // Tăng ID cho CD mới
...
// Hàm cập nhật thông tin của một CD theo tên
void updateCDInfo() {
    char targetName[50];
    printf("Nhap ten CD muon cap nhat thong tin: ");
    scanf(" %49[^\n]", targetName); // Them dau cach truoc % để loại bỏ ký tự '\n' còn lại trong buffer
...
void deleteCD() {
    ...
    if (foundIndex != -1) {
        // Di chuyen cac CD sau ve truoc de xoa CD hien tai
...
// Hàm sắp xếp danh sách CD theo thời lượng tăng dần
...
// Hàm tìm kiếm CD theo tên (sử dụng tìm kiếm nhị phân)
...
// Hàm tìm kiếm và in ra toàn bộ thông tin của các CD theo status
